package com.virtugos.uberapp.parse;
...
import com.virtugos.uberapp.MainActivity;
import com.virtugos.uberapp.R;
import com.virtugos.uberapp.db.DBHelper;
import com.virtugos.uberapp.maputils.PolyLineUtils;
import com.virtugos.uberapp.models.ApplicationPages;
import com.virtugos.uberapp.models.Bill;
...
import com.virtugos.uberapp.models.History;
...
import com.virtugos.uberapp.utils.ReadFiles;
...
//            }
    ...
    public boolean isSuccess(String response) {
        ...
        try {
            ...
            if (jsonObject.getBoolean(KEY_SUCCESS)) {
                ...
                // AndyUtils.showToast(jsonObject.getString(KEY_ERROR),
                // activity);
                if (jsonObject.getJSONArray(KEY_ERROR_MESSAGES).getInt(0) == 11 && !Const.isTokenExpired) {
    ...
    public int checkRequestStatus(String response) {
        ...
        try {
            ...
            preferenceHelper.putBrowserKey(jsonObject.optString(Const.Params.BROWSER_KEY));
            Log.d("checkRequestStatus","***************BROWSER_KEY****************"+preferenceHelper.getBrowserKey());
            preferenceHelper.putStripePublishableKey(jsonObject.optString(Const.Params.STRIPE_PUBLISHABLE_KEY));
            ...
            Log.d("owner","*****************************"+owner);
            if(owner.getString("owner_lat").length() !=0){
                Log.d("owner","*****************************"+owner.getString("owner_lat"));
                preferenceHelper.putClientSource(new LatLng(owner
                        .getDouble("owner_lat"), owner
                        .getDouble("owner_long")));
    ...
    public int getRequestInProgress(String response) {
        ...
        try {
            ...
            if (jsonObject.getBoolean(KEY_SUCCESS)) {
                ...
                Log.d("getRequestInProgress","************BROWSER_KEY***********"+new PreferenceHelper(activity).getBrowserKey());
...
//        listMenu.add(applicationPages);
    ...
    // }
    // }
    public String parseMinFare(String response) {

        if (TextUtils.isEmpty(response))
            return "";
        try {
            return new JSONObject(response).getString(ETA);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return "";
    ...
    public String parseNearestDriverDistanceString(String response) {
        ...
        try {
            JSONArray jsonArray = new JSONObject(response).getJSONArray(ROUTES);
            if (jsonArray.length() > 0) {
                return jsonArray.getJSONObject(0).getJSONArray(LEGS).getJSONObject(0).getJSONObject(DISTANCE).getString(TEXT);
            } else {
                return "";
            }
        } catch (JSONException e) {
