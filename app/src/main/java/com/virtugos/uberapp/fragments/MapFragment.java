...
public class MapFragment extends BaseFragment implements
    ...
    private String estimatedDistanceTxt = "";
    ...
    private ImageView  selectedCard, selectedCash;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        editPromo = (EditText) view.findViewById(R.id.etApplyPromo);
        ...
        editPromo.setOnKeyListener(new OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if ((event.getAction() == KeyEvent.ACTION_DOWN)
                    ...
                    if ((!(editPromo.getText().toString().equals(activity.getResources()
                    ...
                    public void onCameraChange(CameraPosition camPos) {
                        ...
                        if (!isMapTouched) {
                            ...
                            if (!isAddDestination) {
                                ...
                                getAddressFromLocation(camPos.target, etSource, activity);
                            }
                        }
                        isMapTouched = false;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btnActionNotification:
                if(layoutDestination.getVisibility() == View.VISIBLE){
                    tvVehicleType.setVisibility(View.VISIBLE);
                    layoutDestination.setVisibility(View.GONE);
                    sendReqLayout.setVisibility(View.GONE);
                    activity.setIcon(R.drawable.fare_info);
            ...
            case R.id.tvCardNo:
                if (preference.getDefaultCard() == 0) {
                    AndyUtils.showToast(activity.getResources().getString(R.string.no_card),
                            activity);
                    return;
                }
            ...
            case R.id.btnRefSubmit:
                if (etRefCode.getText().length() == 0) {
                    AndyUtils.showToast(
                            activity.getResources().getString(R.string.text_blank_ref_code),
                            activity);
                    return;
                } else {
...
//                .getString(R.string.select_any_resion));
    ...
    private void requestConfirmation(String destAddr) {
        ...
        activity.setIcon(R.drawable.close_payment);
        layoutDestination.setVisibility(View.VISIBLE);
...
//        });
    ...
    private void scheduleTrip() {
        if (!AndyUtils.isNetworkAvailable(activity)) {
            AndyUtils.showToast(
                    activity.getResources().getString(R.string.dialog_no_inter_message),
                    activity);
            return;
        }
    ...
    private void getAddressFromLocation(final LatLng latlng, final EditText et, final Activity activity) {
...
//        showCancellationtDialog(false);
    ...
    private void pickMeUp() {
        if (!AndyUtils.isNetworkAvailable(activity)) {
            AndyUtils.showToast(activity.getResources().getString(R.string.no_internet),
                    activity);
            return;
        ...
        switch (serviceCode) {
            case Const.ServiceCode.CREATE_REQUEST:
                ...
                if (activity.pContent.isSuccess(response)) {
                    ...
                    Log.d("CREATE_REQUEST","************Map Fragment************"+"CREATE_REQUEST");
                ...
                if (activity.pContent.isSuccess(response)) {
                    ...
                    switch (activity.pContent.checkRequestStatus(response)) {
                        ...
                        case Const.IS_WALKER_STARTED:
                            ...
                            stopCheckingStatusUpdate();
                            stopUpdateProvidersLoaction();
                            Log.d("GET_REQUEST_STATUS","************Map Fragment************"+"IS_WALKER_STARTED");
                        ...
                        case Const.IS_WALKER_RATED:
                            ...
                            if (this.isVisible())
                                removeThisFragment();
                            Log.d("GET_REQUEST_STATUS","************Map Fragment************"+"IS_WALKER_RATED");
                        ...
                        case Const.IS_REQEUST_CREATED:
                            if (activity.pHelper.getRequestId() != Const.NO_REQUEST) {
                                Log.d("GET_REQUEST_STATUS","************Map Fragment************"+"IS_REQEUST_CREATED");
            ...
            case Const.ServiceCode.GET_DURATION:
                ...
                if (!TextUtils.isEmpty(response)) {
                    try {
                        ...
                        estimatedDistanceTxt = activity.pContent.parseNearestDriverDistanceString(response);
                        ...
                        if (isSource) {
                            getAddressFromLocation(
                                    new LatLng(points.get(0).latitude,
                                            points.get(0).longitude), etSource, activity);
                        }
                        // else {
                        // getAddressFromLocation(
                        // new LatLng(points.get(0).latitude,
            ...
            case Const.ServiceCode.DRAW_PATH:
                if (!TextUtils.isEmpty(response)) {
                    ...
                    lineOptions.color(activity.getResources().getColor(R.color.color_path)); // #00008B
                    // rgb(0,0,139)
            ...
            case Const.ServiceCode.GET_NEAR_BY:
                ...
                nearByAd = new ArrayAdapter<>(getActivity(),
    ...
    private void cancelFutureRequest(){
        AndyUtils.showCustomProgressDialog(activity, activity.getResources()
                        .getString(R.string.progress_canceling_trip),
                false, null);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (activity.pContent.isSuccess(response)) {
                ...
                switch (activity.pContent.checkRequestStatus(response)) {
                    ...
                    case Const.IS_WALKER_STARTED:
                        // AndyUtils.removeDriverDetailDialog();
                        Log.d("WalkerStatusReceiver","************Map Fragment************"+"IS_WALKER_STARTED");
                        Driver driver = activity.pContent.getDriverDetail(response);
                    ...
                    case Const.IS_WALKER_RATED:
                        stopCheckingStatusUpdate();
                        if (MapFragment.this.isVisible())
                            Log.d("WalkerStatusReceiver","************Map Fragment************"+"IS_WALKER_RATED");
                            activity.gotoRateFragment(activity.pContent
                                    .getDriverDetail(response));
                        // AndyUtils.removeDriverDetailDialog();
                        break;
    ...
    private void showVehicleDetails() {
        if (vehicleDetailDialog == null) {
            if (listType != null && listType.size() > 0) {
                vehicle = (VehicalType) listType.get(selectedPostion);
...
//        sb.append("?sensor=true&key=" + Const.PLACES_AUTOCOMPLETE_API_KEY);
     ...
     * (non-Javadoc)
     *
     * @see com.automated.taxinow.utils..OnLocationReceived#
