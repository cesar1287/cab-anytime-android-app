...
public class PreferenceHelper {
    ...
    private final String SRC_LAT = "src_lat";
    private final String SRC_LNG = "src_lng";
...
//		this.context = context;
    ...
    public void putClientSource(LatLng destination) {
        Editor edit = app_prefs.edit();
        if (destination == null) {
            edit.putString(SRC_LAT, null);
            edit.putString(SRC_LNG, null);
        } else {
            edit.putString(SRC_LAT, String.valueOf(destination.latitude));
            edit.putString(SRC_LNG, String.valueOf(destination.longitude));
        }
        edit.commit();
    }

    public LatLng getClientSource() {
        try {
            if (app_prefs.getString(SRC_LAT, null) != null) {
                return new LatLng(Double.parseDouble(app_prefs.getString(
                        SRC_LAT, "0.0")), Double.parseDouble(app_prefs
                        .getString(SRC_LNG, "0.0")));
            } else {
                return null;
            }
        } catch (NumberFormatException e) {
